...
% Excel file paths for irradiance and temperature profiles
...
%% Step 4: Time-Series Power Flow + ToU Pricing + DR Scheduling
n_hours = 24;
...
B_grid_dynamic = zeros(1, n_hours);  % ToU grid price
B_DG_dynamic = zeros(1, n_hours);    % ToU solar price
B_DR_dynamic = zeros(1, n_hours);    % ToU DR price
DR_reduction_pu = zeros(nbus, n_hours);  % DR reduction per bus

% Time-of-Use (ToU) Pricing Tariffs ($/MWh)
tou_periods = [1, 7; 7, 17; 17, 20; 20, 24];  % Start and end hours: Off-Peak, Shoulder, Peak, Off-Peak
tou_tariffs_grid = [50, 80, 150, 60];  % Grid tariffs ($/MWh)
tou_tariffs_dg = [40, 60, 100, 45];    % Solar (DG) tariffs ($/MWh)
tou_tariffs_dr = [30, 50, 120, 35];    % DR tariffs ($/MWh)

% Assign ToU tariffs to hourly vectors
for i = 1:size(tou_periods, 1)
    start_h = tou_periods(i, 1);
    end_h = tou_periods(i, 2);
    if end_h <= start_h
        end_h = end_h + 24;  % Handle wrap-around for 22:00-24:00
        B_grid_dynamic(start_h:24) = tou_tariffs_grid(i);
        B_grid_dynamic(1:end_h-24) = tou_tariffs_grid(i);
        B_DG_dynamic(start_h:24) = tou_tariffs_dg(i);
        B_DG_dynamic(1:end_h-24) = tou_tariffs_dg(i);
        B_DR_dynamic(start_h:24) = tou_tariffs_dr(i);
        B_DR_dynamic(1:end_h-24) = tou_tariffs_dr(i);
    else
        B_grid_dynamic(start_h:end_h) = tou_tariffs_grid(i);
        B_DG_dynamic(start_h:end_h) = tou_tariffs_dg(i);
        B_DR_dynamic(start_h:end_h) = tou_tariffs_dr(i);
    end
end

% Display ToU Tariffs
fprintf('\nTime-of-Use (ToU) Pricing Tariffs ($/MWh):\n');
fprintf('Period            | Grid  | Solar (DG) | DR\n');
fprintf('---------------------------------------\n');
fprintf('Off-Peak (00:00-07:00, 22:00-24:00) | %.0f    | %.0f       | %.0f\n', tou_tariffs_grid(1), tou_tariffs_dg(1), tou_tariffs_dr(1));
fprintf('Shoulder (07:00-17:00, 20:00-22:00) | %.0f    | %.0f       | %.0f\n', tou_tariffs_grid(2), tou_tariffs_dg(2), tou_tariffs_dr(2));
fprintf('Peak (17:00-20:00)                  | %.0f    | %.0f       | %.0f\n', tou_tariffs_grid(3), tou_tariffs_dg(3), tou_tariffs_dr(3));
...
for t = 1:n_hours
    ...
    % DR Scheduling: Enhance DR during peak ToU period (17:00-20:00)
    if t >= 17 && t <= 20  % Peak period
        for b = 1:nbus
            DR_reduction_pu(b, t) = min(DR_max_percentage * Pd(b) * 1.5, Pd(b));  % Increase DR by 50% during peak
    ...
    % Billing calculation with ToU pricing and DR
    ...
    cost_DG = P_solar_MWh * B_DG_dynamic(t);  % ToU solar cost
    cost_grid = P_grid_MWh * B_grid_dynamic(t);  % ToU grid cost
...
%% Step 7: Plots
% Load Profiles with Reduced Load __ active power reduction
...
sgtitle('24-Hour Real Load Profiles with ToU/DR Reduction');
...
xlabel('Hour of Day'); ylabel('Price ($/MWh)'); title('Time-of-Use Pricing Over 24 Hours');
grid on;
legend('Location', 'best');
hold off;
...
title('Total System Active Power Loading Before and After ToU/DR Implementation');
grid on;
legend('Location', 'best');
hold off;
...
title('Pareto Front: Cost vs. Emissions with Best Point(ToU)');
grid on;
