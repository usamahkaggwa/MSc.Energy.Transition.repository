%% 4-Bus Power Network with Real Load Profiles + Grid Source + CPP Pricing + DR Scheduling + MOPSO
% Uses correct 'ActivePower_W_' syntax; Sheet1; scales short trace to 24h; PU conv; hourly means
% Grid supplies all load with DSM; Implements CPP Pricing, DR scheduling, and MOPSO for cost/emission optimization
clear; clc; close all;
...
bus_data = [
    ...
    2, 3, 0.2,  0.12,  1.00, 0;  % Bus 2: Load (previously PV, now Maize mill)
...
filenames = {
    ...
    'C:/Users/Baymax/Desktop/Mr.Usamah/Profiles/granular/h_pf/interpolated_water_pump_hpf_profile.xlsx', ...  % Bus 2: Water pump (adjusted for Maize mill)
...
fprintf('Ybus Built for 4-Bus System with Grid-Only Supply at Bus 1.\n');
...
fprintf('%d ', cpp_hours);
fprintf('\nHourly Tariffs (Grid, DR):\n');
disp(array2table([hours', B_grid_dynamic', B_DR_dynamic'], ...
    'VariableNames', {'Hour', 'Grid_Tariff', 'DR_Tariff'}));
...
for t = 1:n_hours
    ...
    total_cost(t) = cost_grid + cost_DSM;  % No solar costs
    
    emission_grid = P_grid_MWh * E_grid;  % 952.6 kg/MWh for grid
    total_emission(t) = emission_grid;  % No solar emissions
...
repository = repository * 10000; %% 10,000 xl
...
xlabel('Hour of Day'); ylabel('Power (kW)'); title('Grid Generation Over 24 Hours');
grid on;
legend('Location', 'best');
...
xlabel('Hour of Day'); title('Total System Net Power Consumption Over 24 Hours (Grid + DR)');
grid on;
...
% Total Active Power Loading vs Reduced Loading
