%% 4-Bus Power Network with Real Load Profiles + Solar PV + Grid Source + CPP Pricing + DR Scheduling + MOPSO
...
% Peak: 100 kW (0.001 pu); Implements CPP Pricing, DR scheduling, and MOPSO for cost/emission optimization
clear; clc; close all;
...
% Excel/CSV file paths for irradiance and temperature profiles
...
% Initial billing parameters (base values for reference)
E_grid = 952.6;  % kg/MWh for Grid (CO2: 950, SO2: 0.5, NOx: 2.1)

% CPP and DR parameters
base_price_grid = 70;  % Base grid price ($/MWh)
cpp_multiplier = 3;   % Critical peak price is 3x base price
cpp_threshold_factor = 0.9;  % Trigger CPP if load > 90% of max load
...
% Compute Solar P using simple PV model: P = P_stc * (G/G_stc) * [1 + gamma*(T - T_stc)]
...
%% Step 4: Time-Series Power Flow + CPP Pricing + DR Scheduling
n_hours = 24;
...
B_grid_dynamic = zeros(1, n_hours);  % CPP grid price
B_DG_dynamic = zeros(1, n_hours);    % CPP solar price
B_DR_dynamic = zeros(1, n_hours);    % CPP DR price
DR_reduction_pu = zeros(nbus, n_hours);  % DR reduction per bus

% Calculate total active load for CPP threshold
total_active_load = sum(Pd_profiles, 1);
max_load = max(total_active_load);
cpp_threshold = max_load * cpp_threshold_factor;
cpp_hours = find(total_active_load >= cpp_threshold);  % Hours where load exceeds threshold

% Set CPP prices
B_grid_dynamic(:) = base_price_grid;  % Base price for all hours
B_DG_dynamic(:) = base_price_grid * 0.8;  % Solar base price (80% of grid)
B_DR_dynamic(:) = base_price_grid * 0.6;  % DR base price (60% of grid)
B_grid_dynamic(cpp_hours) = base_price_grid * cpp_multiplier;  % Critical peak price for grid
B_DG_dynamic(cpp_hours) = (base_price_grid * 0.8) * cpp_multiplier;  % Critical peak price for solar
B_DR_dynamic(cpp_hours) = (base_price_grid * 0.6) * cpp_multiplier;  % Critical peak price for DR

% Display CPP Tariffs
fprintf('\nCritical Peak Pricing (CPP) Tariffs ($/MWh):\n');
fprintf('Base Price (Grid): %.0f, Critical Peak (Grid): %.0f (x%.1f)\n', base_price_grid, base_price_grid * cpp_multiplier, cpp_multiplier);
fprintf('Base Price (Solar): %.0f, Critical Peak (Solar): %.0f\n', base_price_grid * 0.8, (base_price_grid * 0.8) * cpp_multiplier);
fprintf('Base Price (DR): %.0f, Critical Peak (DR): %.0f\n', base_price_grid * 0.6, (base_price_grid * 0.6) * cpp_multiplier);
fprintf('CPP Triggered at Hours: ');
fprintf('%d ', cpp_hours);
fprintf('\nHourly Tariffs (Grid, Solar, DR):\n');
disp(array2table([hours', B_grid_dynamic', B_DG_dynamic', B_DR_dynamic'], ...
    'VariableNames', {'Hour', 'Grid_Tariff', 'Solar_Tariff', 'DR_Tariff'}));
...
for t = 1:n_hours
    ...
    % DR Scheduling: Enhance DR during CPP hours
    if ismember(t, cpp_hours)  % Enhance DR during critical peak hours
        for b = 1:nbus
            DR_reduction_pu(b, t) = min(DR_max_percentage * Pd(b) * 1.5, Pd(b));  % Increase DR by 50% during CPP
    ...
    % Billing calculation with CPP pricing and DR
    ...
    cost_DG = P_solar_MWh * B_DG_dynamic(t);  % CPP solar cost
    cost_grid = P_grid_MWh * B_grid_dynamic(t);  % CPP grid cost
    cost_DSM = P_DR_MWh * B_DR_dynamic(t) - (P_DR_MWh * B_DR_dynamic(t) * DR_incentive_factor);  % Cost minus incentive
    total_cost(t) = cost_DG + cost_grid + cost_DSM;
    
    emission_DG = P_solar_MWh * 0;  % 0 for solar
...
v_max = 0.2;  % Increased maximum velocity
...
for iter = 1:n_iterations
    ...
    for i = 1:n_particles
        ...
        for t = 1:n_hours
            dr_reduction_pu_opt(1, t) = particle((t-1)*2+2);  % Base DR from particle
            if ismember(t, cpp_hours)  % Enhance DR during critical peak hours
                dr_reduction_pu_opt(1, t) = min(dr_reduction_pu_opt(1, t) * 1.5, DR_max_percentage * sum(Pd_profiles(:, t)));
            end
        end
        total_cost_i = 0;
        total_emission_i = 0;
...
%% Step 7: Plots
% Load Profiles with Reduced Load
figure('Position', [100, 100, 1200, 800]);
for b = 1:nbus
    subplot(2,2,b);
    yyaxis left;
    plot(hours, Pd_profiles(b,:), 'b-', 'LineWidth', 2, 'DisplayName', 'Original Load (pu)');
    hold on;
    plot(hours, Pd_profiles(b,:) - DR_reduction_pu(b,:), 'r--', 'LineWidth', 2, 'DisplayName', 'Reduced Load (pu)');
    ylabel('Power (pu)');
    % yyaxis right;
    % %plot(hours, Qd_profiles(b,:), 'm--', 'LineWidth', 2, 'DisplayName', 'Reactive Load (pu)');
    % %ylabel('Reactive Power (pu)');
    % title(sprintf('Bus %d Load Profile', b));
    xlabel('Hour of Day');
    grid on;
    legend('Location', 'best');
    hold off;
end
sgtitle('24-Hour Real Load Profiles with CPP/DR Reduction');
...
xlabel('Hour of Day'); ylabel('Price ($/MWh)'); title('Critical Peak Pricing Over 24 Hours');
grid on;
legend('Location', 'best');
hold off;
...
% New Plot: Total Active Power Loading vs Reduced Loading
subplot(2,2,3);
total_active_load = sum(Pd_profiles, 1);  % Total original active power loading
reduced_active_load = total_active_load - sum(DR_reduction_pu, 1);  % Total reduced active power loading
plot(hours, total_active_load, 'b-', 'LineWidth', 2, 'DisplayName', 'Total Active Load (pu)');
hold on;
plot(hours, reduced_active_load, 'r--', 'LineWidth', 2, 'DisplayName', 'Reduced Active Load (pu)');
xlabel('Hour of Day');
ylabel('Total Active Power (pu)');
title('Total System Active Power Loading Before and After CPP/DR Implementation');
grid on;
legend('Location', 'best');
hold off;
...
xlabel('Hour of Day'); title('Objective Functions: Cost and Emissions Over 24 Hours (CPP)');
grid on;
...
title('Pareto Front: Cost vs. Emissions with Best Point (CPP)');
grid on;
...
if ~isempty(best_particle_idx)
    ...
    for t = 1:n_hours
        optimal_dr_reduction_pu(1, t) = optimal_particle((t-1)*2+2);
        if ismember(t, cpp_hours)  % Enhance DR during critical peak hours
            optimal_dr_reduction_pu(1, t) = min(optimal_dr_reduction_pu(1, t) * 1.5, DR_max_percentage * sum(Pd_profiles(:, t)));
        end
        total_load_t = sum(Pd_profiles(:, t));
        dr_reduction_t = min(optimal_dr_reduction_pu(1, t), DR_max_percentage * total_load_t);
        for b = 1:nbus
            optimal_dr_reduction_pu(b, t) = dr_reduction_t * (Pd_profiles(b, t) / total_load_t);
        end
    end
